<x-app-layout>
<x-rh.page-header
    :title="$mode === 'create' ? 'Nouvelle présence' : 'Modifier présence'"
    :breadcrumbs="['Présences' => route('presences.index'), $mode === 'create' ? 'Nouvelle' : 'Modifier' => null]">
    <a href="{{ route('presences.index') }}" class="tb-btn">← Retour</a>
</x-rh.page-header>
...
<form method="POST" action="{{ $mode === 'create' ? route('presences.store') : route('presences.update',$presence) }}" style="display:flex;flex-direction:column;gap:16px;">
@csrf
...
<x-rh.form-section title="Employé & Date">
    ...
    <x-rh.form-field label="Employé" name="employe_id">
        <input type="text" class="form-control" value="{{ $presence->employe->nom_complet }}" disabled>
    </x-rh.form-field>
    <x-rh.form-field label="Date" name="date">
        <input type="text" class="form-control" value="{{ $presence->date->format('d/m/Y') }}" disabled>
    </x-rh.form-field>
    @endif
</x-rh.form-section>
...
<x-rh.form-section title="Horaires & Statut">
    <x-rh.form-field label="Statut" name="statut" :required="true">
        <select name="statut" class="form-control @error('statut') is-invalid @enderror">
            @foreach(\App\Models\Presence::STATUTS as $v => $s)
            <option value="{{ $v }}" {{ old('statut', $presence->statut) === $v ? 'selected' : '' }}>{{ $s['label'] }}</option>
            @endforeach
        </select>
    </x-rh.form-field>
    <x-rh.form-field label="Motif d'absence" name="motif_absence">
        <input type="text" name="motif_absence" class="form-control" value="{{ old('motif_absence', $presence->motif_absence) }}" placeholder="Si absent...">
    </x-rh.form-field>
    <x-rh.form-field label="Heure d'arrivée" name="heure_arrivee">
        <input type="time" name="heure_arrivee" class="form-control" value="{{ old('heure_arrivee', $presence->heure_arrivee ? \Carbon\Carbon::parse($presence->heure_arrivee)->format('H:i') : '') }}">
    </x-rh.form-field>
    <x-rh.form-field label="Heure de départ" name="heure_depart">
        <input type="time" name="heure_depart" class="form-control" value="{{ old('heure_depart', $presence->heure_depart ? \Carbon\Carbon::parse($presence->heure_depart)->format('H:i') : '') }}">
    </x-rh.form-field>
    <x-rh.form-field label="Heures prévues" name="heures_prevues">
        <input type="number" name="heures_prevues" class="form-control" value="{{ old('heures_prevues', $presence->heures_prevues ?? 8) }}" min="0" max="24" step="0.5">
    </x-rh.form-field>
    <x-rh.form-field label="Heures réalisées" name="heures_realisees">
        <input type="number" name="heures_realisees" class="form-control" value="{{ old('heures_realisees', $presence->heures_realisees) }}" min="0" max="24" step="0.5">
    </x-rh.form-field>
</x-rh.form-section>

<x-rh.form-section title="Remarque">
    <x-rh.form-field label="Remarque" name="remarque" :full="true">
        <textarea name="remarque" class="form-control" rows="3" style="height:auto;padding:8px 10px;">{{ old('remarque', $presence->remarque) }}</textarea>
    </x-rh.form-field>
</x-rh.form-section>

<div style="display:flex;justify-content:flex-end;gap:10px;">
    <a href="{{ route('presences.index') }}" class="tb-btn">Annuler</a>
